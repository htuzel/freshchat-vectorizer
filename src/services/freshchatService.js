...
class FreshchatService {
  constructor() {
    const baseURL = `https://${config.freshchat.apiUrl}`;
    ...
    try {
      ...
      for (let i = 0; i < users.length; i++) {
        ...
        try {
          ...
          const conversationsCount = await this.getUserAndStoreConversations(
            user,
            fromDate
          );
          ...
          console.log(
            `Progress: ${i + 1}/${
              users.length
            } users processed, ${totalConversations} total conversations stored`
          );
        } catch (error) {
  ...
  async getUserAndStoreConversations(userId, fromDate) {
    ...
    while (true) {
      try {
        ...
        console.log(
          `Processing ${userConversations.length} conversations for user ${userId} (page ${page})`
        );
        ...
        for (let conversation of userConversations) {
          try {
            ...
            console.error(
              `Error processing conversation ${conversation.id} for user ${userId}:`,
              error
            );
          }
        }
        ...
        console.log(
          `Stored ${userConversations.length} conversations for user ${userId} (page ${page})`
        );
  ...
  formatConversation(rawConversation) {
    try {
      ...
      const messages = (rawConversation.messages || [])
        ...
        .sort(
          (a, b) =>
            new Date(a.timestamp || a.created_time) -
            new Date(b.timestamp || b.created_time)
        )
        .map((msg) => {
